...
import com.businesshub.be.payload.request.SignupRequest;
import com.businesshub.be.payload.response.JwtResponse;
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "admin":
                        ...
                        break;
                    case "businessowner":
                        Role businessownerRole = roleRepository.findByName(ERole.ROLE_BUSINESSOWNER)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
